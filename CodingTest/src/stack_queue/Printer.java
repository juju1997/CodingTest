...
//		int location = 2;
		...
		int location = 0;
		System.out.println(solution(priorities, location) + " answer ");
	...
	public static int solution(int[] priorities, int location) {
        ...
        Queue<Print> q = new LinkedList<>();	//인쇄전 
        Queue<Print> succ = new LinkedList<>();	//인쇄후 
        ...
        while(!q.isEmpty()) {
        	comp=0;
        	if(q.size() == 1) {	//for문 못돌리는거 방지 
        		succ.offer(q.poll());
        	}else {
        		Print p = q.poll();
        		for(Print comP : q) {
        			if(p.priority < comP.priority) {	//큰놈이 있다? 
        				q.offer(p);
        				comp++;
        				break;
        			}
